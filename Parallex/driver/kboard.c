/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: 2016年08月03日 星期三 21时17分11秒
 ************************************************************************/
...
typedef uint8 key_status_t;

typedef
struct keymap {
        uint8 scancodes[128];         // 键盘扫描码的映射
        uint8 capslock_scancodes[128];
        uint8 shift_scancodes[128];
        uint8 control_map[8];
...
// 设备是否可用
static uint32 kb_device_valid(void);

// 获取设备描述
static const char *kb_get_desc(void);

// 设备读取
static int kb_read(void *dec, uint32 len);

// 设备写入
static int kb_write(const void *src, uint32 len);

// 设备控制
static int kb_ioctl(int op, int flag);

// 键盘中断处理函数
static void keyboard_handler( pt_regs_t *regs);
...
static struct kboard_device {
        uint32 is_valid;                   // 设备是否可用
        keymap_t *curr_layout;           // 当前的字符集
        uint8 kbuffer[KBUFFER_LEN];    // 键盘输入的缓冲区队列
        uint32 kfront;                 // 缓冲队列队头
        uint32 krear;                  // 缓冲队列队尾
} kb_device;
...
static void keyboard_handler( pt_regs_t *regs)
{
        ...
        // 当键被按下
                ...
                uint8 *scancodes = layout->scancodes;
