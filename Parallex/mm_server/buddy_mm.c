/*************************************************************************
	> File Name: buddy_mm.c
	> Author: 
	> Mail: 
	> Created Time: 2016年07月26日 星期二 08时39分19秒
 ************************************************************************/
...
static void buddy_page_init(page_t *pages, uint32 n);
static uint32 buddy_alloc_pages(uint32 n);
static void buddy_free_pages(uint32 addr, uint32 n);
static uint32 buddy_free_pages_count(void);
...
static uint32 get_order(uint32 size)
{
        uint32 order = 0, order_size = 1;
        ...
        printk("getorder failed.");
        return 0;
}

static page_t *buddy_alloc_pages_sub(uint32 order)
{
        assert(order <= MAX_ORDER, "buddy_alloc_pages_sub cannot alloc over 1024 pages");

        for (uint32 cur_order = order; cur_order <= MAX_ORDER; ++cur_order) {
...
static uint32 buddy_alloc_pages(uint32 n)
{
        if (n <= 0 || n > (uint32)atomic_read(&buddy_mm_info.phy_page_now_count)) {
                return 0;
        }

        uint32 order = get_order(n);
        uint32 order_size = (1u << order);
...
static inline uint32 page_to_idx(page_t *page)
{
        return (page - buddy_mm_info.pages_base);
}

static inline page_t *idx_to_page(uint32 idx)
{
        return (buddy_mm_info.pages_base + idx);
}

static uint32 page_is_buddy(page_t *page, uint32 order)
{
        if (page_to_idx(page) < (uint32)atomic_read(&buddy_mm_info.phy_page_count)) {
...
static void buddy_free_pages_sub(page_t *base, uint32 order)
{
        uint32 buddy_idx, page_idx = page_to_idx(base);
...
static void buddy_show_management_info(void)
{
        printk("Buddy Memory Info:\n\n");
        uint32 count = 0;
        for (uint32 order = 0; order < MAX_ORDER + 1; ++order) {
                count += atomic_read(&nr_free(order)) * (1u << order);
                printk("    order %2u (size: %4u * 4K) count: %3d\n",
...
static void buddy_test_mm(void)
{
          ...
          printk("before test:\n\n order 0~10 counts: ");
        for (uint32 order = 0; order < MAX_ORDER + 1; ++order) {
                printk("%3u ", atomic_read(&nr_free(order)));
        }
        printk("\n\n");
